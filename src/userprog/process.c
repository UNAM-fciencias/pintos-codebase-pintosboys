...
tid_t process_execute(const char *file_name)
{
  ...
  sema_init(&cur->loaded, 0);
  sema_down(&cur->loaded);
...
start_process(void *file_name_)
{
  ...
  struct thread *cur = thread_current();
  sema_up(&cur->father->loaded);

  /* If load failed, quit. */

  if (!success)
  {
    cur->father->successfully_load = false;
  ...
  else
  {
    ...
    cur->father->successfully_load = true;
  }
